...
import { calculateAngle, smoothAngle, determineFeedbackSeverity } from './pose-utils.js';
...
class FormVisualizer {
  constructor(canvasElement, canvasContext) {
    ...
    this.idealPose = null;
    this.formIssues = [];
    this._exerciseMetricsData = null;
  }

  // Update the visualizer with new pose data and form analysis
  // exerciseMetricsData is passed in by the app instead of imported directly
  update(poseData, formQuality, formIssues, exerciseType, exerciseMetricsData) {
    ...
    if (exerciseMetricsData !== undefined) {
      this._exerciseMetricsData = exerciseMetricsData;
    }
  ...
  drawFormCorrections() {
    ...
    this.formIssues.forEach(issue => {
      if (issue.includes("elbow")) {
        this.highlightJoint(pose[14], this.colors.error, "Fix elbow position");
        this.highlightJoint(pose[13], this.colors.error, "Fix elbow position");
      } else if (issue.includes("shoulder")) {
        this.highlightJoint(pose[12], this.colors.error, "Level shoulders");
        this.highlightJoint(pose[11], this.colors.error, "Level shoulders");
      } else if (issue.includes("wrist")) {
        this.highlightJoint(pose[16], this.colors.error, "Align wrist");
        this.highlightJoint(pose[15], this.colors.error, "Align wrist");
      }
    });
  ...
  drawAngleVisualizations(metrics) {
    ...
    // Use injected exercise metrics data
    const exerciseMetricsData = this._exerciseMetricsData;
        ...
        if (metrics.jointAngles && metrics.jointAngles[metricKey]) {
          ...
          if (measureResult && measureResult.idealRange && measureResult.feedbackText) {
            ...
            if (severity === "error" && measureResult.feedbackText.error) {
              feedbackMessages.push({ text: measureResult.feedbackText.error, severity: "error" });
            } else if (severity === "warning" && measureResult.feedbackText.warning) {
              feedbackMessages.push({ text: measureResult.feedbackText.warning, severity: "warning" });
            } else if (severity === "good" && measureResult.feedbackText.good) {
              feedbackMessages.push({ text: measureResult.feedbackText.good, severity: "good" });
            }
          }
        }
      });
